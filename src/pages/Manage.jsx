          ...
          <>
            {modules.length === 0 && (
              <div className="text-center py-10 mb-4">
                ...
                <button onClick={handleSeedDefaults} className="text-[15px] font-semibold px-5 py-2.5 rounded-xl" style={{ backgroundColor: '#FFCC00', color: '#7a6500' }}>
                  Add default modules
                </button>
              </div>
            )}
    ...
    <div className="flex gap-1 p-1 rounded-xl" style={{ backgroundColor: '#e5e5ea' }}>
      {options.map((opt) => (
        <button
          ...
          style={value === opt ? { backgroundColor: '#FFCC00', color: '#7a6500', boxShadow: '0 1px 3px rgba(0,0,0,0.12)' } : { color: '#636366' }}
        >
          {opt}
        </button>
      ))}
        ...
        <div className="flex flex-wrap gap-2">
          {MODULE_ICONS.map((ic) => (
            <button key={ic} onClick={() => setIcon(ic)}
              ...
              style={icon === ic ? { backgroundColor: '#FFF9E6', outline: '2px solid #FFCC00' } : { backgroundColor: '#f2f2f7' }}
            >{ic}</button>
          ))}
        </div>
      </Field>
      ...
      {taskType === 'scheduled' && (
        <>
          ...
          <div className="flex items-center justify-between card px-4 py-3.5 mb-5 cursor-pointer" onClick={() => setIsReschedulable(!isReschedulable)}>
            ...
            <div className="w-12 h-7 rounded-full relative transition-colors shrink-0" style={{ backgroundColor: isReschedulable ? '#FFCC00' : '#e5e5ea' }}>
